---
layout: mentor
title: About My Mentors
permalink: /about-my-mentor.html

graduate_mentor:
  name: Derrick Mirindi
  title: PhD Candidate, Computer Engineering
  image: /assets/images/graduate-student-mentor.svg
  website: (https://morganstateuniversity365-my.sharepoint.com/:p:/g/personal/yuhan_jiang_morgan_edu/IQBvdlmmNm2BR7ip4qZWW72IAddhXpfE2NQan-YC4A2pQqQ?rtime=Vu2CJgC83kg)
  bio: |
    Working ith Dr. Yuhan Jiang. Trying to detect defects on  buildings and time of performance of the degration of those buildings. Using algoithms such as Yuno to detect defects. This is a project in Baltimore. 

    Outside of research, Alex organizes weekly reading groups and is a strong advocate
    for undergraduate research participation.

faculty_mentor:
  name: Dr. Yuhan Jiang
  title: Associate Professor, School of Engineering
  image: /assets/images/faculty-mentor.svg
  website: https://example.com/pat-nguyen
  bio: |
    Dr. Nguyen leads a research group focused on developing edge AI applications for human-robot collaboration in LEGO assembling operations. Their guidance will helped me sharpen my research questions and
    skills. I love legos, so I really want to learn about AI in relation to LEGOs.
    Background: Civil Engineering. They look for cracks on pavement, defects with windows, etc. They try to locate how many pixels and convert it into square feet. They are relating this to our project. 

    Dr. Yuhan Jiang has mentored many SAIRI students and has a variety of experience in the project he is requesting to do.
---
